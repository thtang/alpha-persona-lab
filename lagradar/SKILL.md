...
Optional local MLX embeddings for ThemeMiner relation judgments:
...
Lagradar consumes `relation_quality_score`, `relation_authority`, `semantic_similarity`, `lexical_similarity`, and optional `embedding_similarity` from ThemeMiner. Treat embeddings as a relation-quality input, not a standalone buy signal; the final pick still needs peer challenge, price/volume confirmation, and risk gates.

`scan_laggards.py` automatically syncs the latest ThemeMiner graph from `thememiner/output/` by default. It merges `theme_library.json`, `relation_index.json`, `company_profiles.json`, `company_thesis_cards.json`, and `relation_judgments.json` into the scan, then writes `lagradar/output/synced_theme_seed.json` and ranked candidates with thesis labels, AI-chain positions, catalysts, leader indicators, peer symbols, agent status, relation authority, relation quality, semantic similarity, and risks. Use `--no-sync-thememiner` only when debugging the old handcrafted seed.

If ThemeMiner produced `agent_status=openai_agent_unavailable_no_api_key/codex_agent_unavailable_no_cli`, discovery produced `match_authority=rule_fallback`, or the relation index labels a row `relation_authority=fallback_recall_only`, treat those mappings as fallback evidence only. The scanner and selector penalize these rows; do not promote them over `agent_applied`, `manual_override`, or `profile_supported` names without extra source verification.
...
- `../thememiner/output/relation_judgments.json`: stock-theme authority layer generated by `build_semantic_relation_index.py`; contains `relation_quality_score`, `relation_authority`, `semantic_similarity`, matched terms, evidence paths, and warnings.
...
- `../thememiner/data/data_source_registry_seed.json`: upstream source registry that explains which official, market, flow, physical-constraint, macro, and social-lead sources are required to prove a proactive theme. Use it when a candidate needs better evidence rather than patching rankings with labels.
...
- `data/backtest_market_universe.json`: market, sector, commodity, rates, and FX proxies for 20-year backtests and controls, including Korea proxies such as EWY, KOSPI, KOSDAQ, KODEX 200, KODEX KOSDAQ 150, KODEX Semiconductor, and USD/KRW.
...
3. Run `scan_laggards.py` to compute:
   ...
   - `relation_quality_score`: direct business-fit authority from manual overrides, agent/card quality, profile support, semantic similarity, and fallback warnings.
...
- If a theme lacks strong/medium source support, consult ThemeMiner's data-source registry and identify the missing source family: company truth, physical constraint, market validation, flow/chips, macro liquidity, or weak lead.
